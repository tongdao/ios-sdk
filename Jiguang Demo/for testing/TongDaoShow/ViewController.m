...
- (void)viewDidLoad {
    ...
    [TongDao sharedManager].registerOnRewardBeanUnlocked = ^(NSArray* rewards){
    ...
    [[TongDao sharedManager]trackWithEventName:@"open-message"];
...
-(void)btnClick:(UIButton*)sender
{
    ...
    if (bean.type == EVENT) {
        if (bean.datas.count == 0) {
            [[TongDao sharedManager] trackWithEventName:bean.eventName];
        } else {
            [[TongDao sharedManager] trackWithEventName:bean.eventName andValues:bean.datas];
        }
    } else if (bean.type == ATTRIBUTE) {
        if (bean.datas.count != 0) {
            [[TongDao sharedManager] identify:bean.datas];
        }
    }
...
-(void)touchBtnMethod:(UIButton*)sender
{
    ...
    if (sendTransferBean.type == EVENT) {
        if (sendTransferBean.datas.count == 0) {
            [[TongDao sharedManager] trackWithEventName:sendTransferBean.eventName];
        }else {
            [[TongDao sharedManager] trackWithEventName:sendTransferBean.eventName andValues:sendTransferBean.datas];
        }
    }else if (sendTransferBean.type == ATTRIBUTE) {
        if (sendTransferBean.datas.count > 0) {
            [[TongDao sharedManager] identify:sendTransferBean.datas];
        }
    }
}
