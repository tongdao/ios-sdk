...
-(void)sendInitialDataAndIgnore:(TongDaoinitData)ingnoreInfor{
    ...
    if (ingnoreInfor == TDLocationIfor) {
        ...
        TdAppInfoTool *infoTool = [[TdAppInfoTool alloc]init];
...
//        NSLog(@"open message%@",data);
        [TdApiTool postEvents:data callBackForOpenMessage:self];
    }
}
...
-(void)postEventData:(NSMutableArray*)arr{
    if (self.failTrackList != nil&&self.failTrackList.count>0&&self.isFailed == YES) {
        ...
        if (data != nil) {
//            NSLog(@"%@",data);
            [TdApiTool postEvents:data callBack:self];
        }
...
//    NSLog(@"apiCallback回调");
    ...
    if (httpRes == nil) {
        ...
        if (resCode==200||resCode==204) {
            ...
            if (resErr==nil) {
                if (jsonRes==nil) {
                    if ([TdBridge sharedTdBridge].onErrorListener != nil) {
                        [[TdBridge sharedTdBridge].onErrorListener onError:[[TdErrorTool sharedTdErrorTool] makeErrorBeanWithErrorCode:resCode andErrorMessage:ErrorInfos[errorMessageJson]]];
                    }else{
                        NSLog(@"resCode is %ld",(long)resCode);
                        [self addTrackEvent:sendData];
                                self.isFailed = YES;
                    ...
                    if ([TdBridge sharedTdBridge].onErrorListener != nil) {
                        [[TdBridge sharedTdBridge].onErrorListener onError:[[TdErrorTool sharedTdErrorTool] makeErrorBeanWithErrorCode:resCode andErrorMessage:message]];
                    }else{
                        NSLog(@"resCode is %ld",(long)resCode);
                        [self addTrackEvent:sendData];
                                self.isFailed = YES;
